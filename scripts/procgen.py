...
def place_entities(
    ...
    for i in range(number_of_items):
        ...
        if not any(entity.x == x and entity.y == y for entity in dungeon.entities):
            ...
            if item_chance < 0.5:
                scripts.entity_factories.health_potion.spawn(dungeon, x, y)
            elif item_chance < 0.8:
                scripts.entity_factories.fireball_scroll.spawn(dungeon, x, y)
            elif item_chance < 0.9:
                scripts.entity_factories.confusion_scroll.spawn(dungeon, x, y)
            else:
                scripts.entity_factories.lightning_scroll.spawn(dungeon, x, y)
